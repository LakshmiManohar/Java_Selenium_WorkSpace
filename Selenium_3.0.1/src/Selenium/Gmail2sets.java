...
public class Gmail2sets {
  ...
  public void f() throws Exception {
	  ...
	  Sheet s= wb.getSheet(0);
		  ...
		  for(int j=4;j<s.getColumns();j++)
	  {
			  for(int i=0;i<s.getRows();i++)
		  {
			  if(s.getCell(3,i).getContents().equalsIgnoreCase("URL"))
			  {
				  dr.get(s.getCell(j,i).getContents());
			  }
			  
			  else if (s.getCell(3,i).getContents().equalsIgnoreCase("sendkeys"))
			  {
				  dr.findElement(By.id(s.getCell(1,i).getContents())).sendKeys(s.getCell(j,i).getContents());
			  }
